...
class PygStructureDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        # self.ids = self.df[id_tag] # IT comment out
        # self.atoms = self.df['atoms']
        self.labels = torch.tensor(self.df[target]).type(torch.get_default_dtype())
        # print("mean %f std %f" % (self.labels.mean(), self.labels.std())) # editted
        if mean_train == None:
            ...
            if not eval:  # Note: IT added
                print(
                    "normalize using training mean %f and std %f"
    ...
    def _get_attribute_lookup(atom_features: str = "cgcnn"):
        ...
        for element, v in chem_data.items():
            if element not in ["Md", "No", "Lr"]:  # Note: IT added
                z = v["Z"]
...
def compute_bond_cosine(v1, v2):
    ...
    bond_cosine = torch.sum(v1 * v2) / (torch.norm(v1) * torch.norm(v2))
