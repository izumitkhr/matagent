...
class ComformerConv_edge(nn.Module):
    def __init__(
        ...
        self.lin_edge_len = nn.Linear(
            in_channels[0] + self.embedding_dim, in_channels[0]
        )
        self.lin_concate = nn.Linear(heads * out_channels, out_channels)
        self.lin_msg_update = nn.Sequential(
            nn.Linear(out_channels * 3, out_channels),
...
class TensorProductConvLayer(torch.nn.Module):
    ...
    def __init__(
        self, in_irreps, sh_irreps, out_irreps, n_edge_features, residual=True
    ):
...
class ComformerConvEqui(nn.Module):
    ...
    def forward(
        ...
        node_feature = self.softplus(
            self.node_linear_2(self.softplus(self.bn(node_feature)))
        )
